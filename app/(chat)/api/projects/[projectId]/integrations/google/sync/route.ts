...
import { syncGoogleDriveItemsToProjectDocs } from "@/lib/integrations/google/sync-google-docs";
...
export async function GET(
  ...
  try {
    ...
    // Filter for docs that have Google metadata
    const syncedDocs = docs
      .filter((doc) => {
        const meta = doc.metadata as Record<string, unknown> | null;
        return meta && typeof meta.googleFileId === "string";
      })
      .map((doc) => {
        ...
        return {
          ...
          fileId: meta.googleFileId as string,
          lastSyncedAt:
...
export async function POST(
  ...
  const results = await syncGoogleDriveItemsToProjectDocs({
    userId: session.user.id,
    project,
